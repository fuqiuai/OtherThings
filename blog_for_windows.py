...
# 设置打开网页的浏览器
firefoxPath = r'C:\Program Files\Mozilla Firefox\firefox.exe'            #  例如我的：C:\***\***\***\***\Google\Chrome\Application\chrome.exe 
webbrowser.register('firefox', None, webbrowser.BackgroundBrowser(firefoxPath))  #这里的'firefox'可以用其它任意名字，这里将想打开的浏览器保存到'firefox'

for j in range(0,300):#设置循环的总次数   
    i=0    
    while i<1 :  #一次打开浏览器访问的循环次数
        ...
        os.system('taskkill /F /IM firefox.exe')  #关闭进程。其他的更换下就行 #/F强制关闭进程  /T关闭的进程树及子树  /IM进程的映像名称
